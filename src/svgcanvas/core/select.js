...
export class Selector {
  ...
  resize(bbox) {
    ...
    while (currentElt.parentNode) {
      if (
        currentElt.parentNode &&
        currentElt.parentNode.tagName === "g" &&
        currentElt.parentNode.transform
      ) {
        if (currentElt.parentNode.transform.baseVal.numberOfItems) {
          parentTransformationMatrix = matrixMultiply(
            transformListToTransform(getTransformList(selected.parentNode))
              .matrix,
    ...
    const m = parentTransformationMatrix
      ? matrixMultiply(
          parentTransformationMatrix,
